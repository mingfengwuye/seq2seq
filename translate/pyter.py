...
"""
This code comes from the `pyter` module (https://pypi.python.org/pypi/pyter)
"""


def ter(hypothesis, reference, **kwargs):
    """Calculate Translation Error Rate
    hypothesis and references are both lists
    ...
    '0.308'
    """
    inputwords, refwords = list(hypothesis), list(reference)
...
def _findpairs(ws1, ws2):
    for i1, i2 in itertools.product(range(len(ws1)), range(len(ws2))):
        if i1 == i2:
            continue
        if ws1[i1] == ws2[i2]:
            length = 1
...
class CachedEditDistance(object):
    ...
    def _edit_distance(self, iwords, spos, cache):
        ...
        for i, j in itertools.product(range(1, len(iwords) - spos + 1), range(len(self.rwds) + 1)):
            if j == 0:
                l[i][j] = l[i - 1][j] + 1
            else:
                l[i][j] = min(l[i - 1][j] + 1,
