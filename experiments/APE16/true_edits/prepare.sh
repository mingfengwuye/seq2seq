...
data_dir=experiments/APE16/true_edits/data
...
scripts/extract-edits.py ${data_dir}/train.raw.{mt,pe} --ops-only > ${data_dir}/train.raw.edits.ops
scripts/extract-edits.py ${data_dir}/dev.raw.{mt,pe} --ops-only > ${data_dir}/dev.raw.edits.ops
scripts/extract-edits.py ${data_dir}/test.raw.{mt,pe} --ops-only > ${data_dir}/test.raw.edits.ops

scripts/extract-edits.py ${data_dir}/train.raw.{mt,pe} --word > ${data_dir}/train.raw.edits.words
scripts/extract-edits.py ${data_dir}/dev.raw.{mt,pe} --words > ${data_dir}/dev.raw.edits.words
scripts/extract-edits.py ${data_dir}/test.raw.{mt,pe} --words > ${data_dir}/test.raw.edits.words
